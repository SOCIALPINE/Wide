...
stdout에는 **프로그램의 출력만** 나갑니다(print/cout). 배너·조명 리포트·에러·`--time`은 전부
stderr입니다 — 채점 서버나 파이프라인에서 답만 깨끗하게 받을 수 있습니다:

```bash
wide prog.wide > answer.txt 2> illumination.txt   # 출력과 조명을 따로
wide --no-illum prog.wide                          # 조명 리포트 끄기
```
